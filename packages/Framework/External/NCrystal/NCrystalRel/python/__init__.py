...
In particular, a small example using the NCrystal python module can be found at:
   ...
   https://github.com/mctools/ncrystal/blob/master/examples/ncrystalrel_example_py
...
For detailed usage conditions and licensing of this open source project, see:
   ...
   https://github.com/mctools/ncrystal/blob/master/ncrystalrel_extra/LICENSE
...
def _load(nclib_filename):
    ...
    _wrap('ncrystalrel_decodecfg_packfact',_dbl,(_cstr,))
    _wrap('ncrystalrel_decodecfg_vdoslux',_uint,(_cstr,))
    _wrap('ncrystalrel_disable_caching',None,tuple())
    _wrap('ncrystalrel_enable_caching',None,tuple())
...
def decodecfg_packfact(cfgstr):
    ...
    return float(_rawfct['ncrystalrel_decodecfg_packfact'](_str2cstr(cfgstr)))

def decodecfg_vdoslux(cfgstr):
    """Extract vdoslux value from cfgstr"""
    return int(_rawfct['ncrystalrel_decodecfg_vdoslux'](_str2cstr(cfgstr)))
...
class RCBase:
    ...
    def __init__(self, rawobj):
        """internal usage only"""
        self._rawobj = rawobj
        #do not ref here, since ncrystalrel_create_xxx functions in C-interface already did so.
        self._rawunref = _rawfct['ncrystalrel_unref']#keep fct reference
    ...
    def refCount(self):
        ...
        return _rawfct['ncrystalrel_refcount'](self._rawobj)
...
class AtomData(RCBase):
    ...
    def __init__(self,rawobj):
        ...
        f=_rawfct['ncrystalrel_atomdata_getfields'](rawobj)
        self.__m = f['m']
    ...
    def getComponent(self,icomponent):
        ...
        if c:
            return c
        rawobj_subc,fraction=_rawfct['ncrystalrel_atomdata_createsubcomp'](self._rawobj,icomponent)
...
class Info(RCBase):
    ...
    def __init__(self, cfgstr):
        ...
        if isinstance(cfgstr,tuple) and len(cfgstr)==2 and cfgstr[0]=='_rawobj_':
            #Already got an ncrystalrel_info_t object:
            rawobj = cfgstr[1]
        else:
            rawobj = _rawfct['ncrystalrel_create_info'](_str2cstr(cfgstr))
    ...
    def _initComp(self):
        ...
        nc = _rawfct['ncrystalrel_info_ncomponents'](self._rawobj)
        self.__comp = []
        for icomp in range(nc):
            atomidx,fraction = _rawfct['ncrystalrel_info_getcomp'](self._rawobj,icomp)
    ...
    def stateOfMatter(self):
        ...
        if self._som is None:
            self._som = StateOfMatter(_rawfct['ncrystalrel_info_getstateofmatter'](self._rawobj))
        return self._som
    ...
    def dump(self):
        ...
        sys.stdout.flush()
        sys.stderr.flush()
        _rawfct['ncrystalrel_dump'](self._rawobj)

    def hasTemperature(self):
        """Whether or not material has a temperature available"""
        return _rawfct['ncrystalrel_info_gettemperature'](self._rawobj)>-1

    def getTemperature(self):
        """Material temperature (in kelvin)"""
        t=_rawfct['ncrystalrel_info_gettemperature'](self._rawobj)
        nc_assert(t>-1)
        return t
    ...
    def _provideAtomData(self,atomindex):
        ...
        if obj:
            return obj
        raw_ad = _rawfct['ncrystalrel_create_atomdata'](self._rawobj,atomindex)
        obj = AtomData(raw_ad)
    ...
    def __initAtomInfo(self):
        ...
        natoms = _rawfct['ncrystalrel_info_natominfo'](self._rawobj)
        hasmsd = bool(_rawfct['ncrystalrel_info_hasatommsd'](self._rawobj))
        hasperelemdt=False
        l=[]
        for iatom in range(natoms):
            atomidx,n,dt,msd = _rawfct['ncrystalrel_info_getatominfo'](self._rawobj,iatom)
            if dt:
                hasperelemdt=True
            assert hasmsd == (msd>0.0)
            pos=[]
            for ipos in range(n):
                pos.append( _rawfct['ncrystalrel_info_getatompos'](self._rawobj,iatom,ipos) )
    ...
    def hasHKLInfo(self):
        ...
        return bool(_rawfct['ncrystalrel_info_nhkl'](self._rawobj)>-1)
    def nHKL(self):
        """Number of HKL planes available (grouped into families with similar d-spacing and f-squared)"""
        return int(_rawfct['ncrystalrel_info_nhkl'](self._rawobj))
    def hklDLower(self):
        """Lower d-spacing cutoff (angstrom)."""
        return float(_rawfct['ncrystalrel_info_hkl_dlower'](self._rawobj))
    def hklDUpper(self):
        """Upper d-spacing cutoff (angstrom)."""
        return float(_rawfct['ncrystalrel_info_hkl_dupper'](self._rawobj))
    def hklList(self):
        """Iterator over HKL info, yielding tuples in the format
        (h,k,l,multiplicity,dspacing,fsquared)"""
        nc_assert(self.hasHKLInfo())
        h,k,l,mult,dsp,fsq = _rawfct['ncrystalrel_info_gethkl_setuppars']()
        for idx in range(self.nHKL()):
            _rawfct['ncrystalrel_info_gethkl'](self._rawobj,idx,h,k,l,mult,dsp,fsq)
    ...
    def dspacingFromHKL(self, h, k, l):
        ...
        return float(_rawfct['ncrystalrel_info_dspacing_from_hkl'](self._rawobj,h,k,l))
    ...
    class DI_ScatKnl(DynamicInfo):
        ...
        def _loadKernel( self, vdoslux = 3 ):
            ...
            if self.__lastvdoslux != vdoslux:
                sugEmax,ne,na,nb,eptr,aptr,bptr,sabptr = _rawfct['ncrystalrel_dyninfo_extract_scatknl'](self._key,vdoslux)
    ...
    class DI_VDOS(DI_ScatKnl):
        ...
        def vdosData(self):
            ...
            if self.__vdosdata is None:
                emin,emax,nd,dptr = _rawfct['ncrystalrel_dyninfo_extract_vdos'](self._key)
        ...
        def __loadVDOSOrig(self):
            if self.__vdosorig is None:
                neg,egptr,nds,dsptr = _rawfct['ncrystalrel_dyninfo_extract_vdos_input'](self._key)
    ...
    class DI_VDOSDebye(DI_ScatKnl):
        ...
        def debyeTemperature(self):
            ...
            if self.__debyetemp is None:
                self.__debyetemp = _rawfct['ncrystalrel_dyninfo_extract_vdosdebye'](self._key)
    ...
    def hasDynamicInfo(self):
        ...
        return int(_rawfct['ncrystalrel_info_ndyninfo'](self._rawobj))>0 if self.__dyninfo is None else bool(self.__dyninfo)

    def getDynamicInfoList(self):
        """Get list of DynamicInfo objects (if available). One for each atom."""
        if self.__dyninfo is None:
            self.__dyninfo = []
            for idx in range(int(_rawfct['ncrystalrel_info_ndyninfo'](self._rawobj))):
                key = (self._rawobj,idx)
                fr,tt,atomidx,ditype = _rawfct['ncrystalrel_dyninfo_base'](key)
    ...
    def getAllCustomSections(self):
        ...
        if self.__custom is None:
            self.__custom = _rawfct['ncrystalrel_info_getcustomsections'](self._rawobj)
...
class CalcBase(RCBase):
    ...
    def getCalcName(self):
        """Calculator name"""
        return _cstr2str(_rawfct['ncrystalrel_name'](self._rawobj))
    @property
    def name(self):
...
class Process(CalcBase):
    ...
    def domain(self):
        ...
        """
        return _rawfct['ncrystalrel_domain'](self._rawobj)
    def isNonOriented(self):
        """opposite of isOriented()"""
        return bool(_rawfct['ncrystalrel_isnonoriented'](self._rawobj))
    def isOriented(self):
    ...
    def crossSection( self, ekin, direction ):
        ...
        return _rawfct['ncrystalrel_crosssection'](self._rawobj,ekin, direction)
    def crossSectionIsotropic( self, ekin, repeat = None ):
        ...
        """
        return _rawfct['ncrystalrel_crosssection_nonoriented'](self._rawobj,ekin,repeat)
...
class Absorption(Process):
    ...
    def __init__(self, cfgstr):
        ...
        if isinstance(cfgstr,tuple) and len(cfgstr)==2 and cfgstr[0]=='_rawobj_':
            ...
            rawobj_abs = _rawfct['ncrystalrel_create_absorption'](_str2cstr(cfgstr))
        self._rawobj_abs = rawobj_abs
        rawobj_proc = _rawfct['ncrystalrel_cast_abs2proc'](rawobj_abs)
    ...
    def clone(self):
        ...
        """
        newrawobj = _rawfct['ncrystalrel_clone_absorption'](self._rawobj_abs)
...
class Scatter(Process):
    ...
    def __init__(self, cfgstr):
        ...
        if isinstance(cfgstr,tuple) and len(cfgstr)==2 and cfgstr[0]=='_rawobj_':
            #Already got an ncrystalrel_scatter_t object:
            self._rawobj_scat = cfgstr[1]
        else:
            self._rawobj_scat = _rawfct['ncrystalrel_create_scatter'](_str2cstr(cfgstr))
        rawobj_proc = _rawfct['ncrystalrel_cast_scat2proc'](self._rawobj_scat)
    ...
    def clone(self,rng_stream_index=None,for_current_thread=False):
        ...
        if rng_stream_index is not None:
            ...
            newrawobj = _rawfct['ncrystalrel_clone_scatter_rngbyidx'](self._rawobj_scat,int(rng_stream_index))
        elif for_current_thread:
            newrawobj = _rawfct['ncrystalrel_clone_scatter_rngforcurrentthread'](self._rawobj_scat)
        else:
            newrawobj = _rawfct['ncrystalrel_clone_scatter'](self._rawobj_scat)
    ...
    def sampleScatter( self, ekin, direction, repeat = None ):
        ...
        """
        return _rawfct['ncrystalrel_samplesct'](self._rawobj_scat,ekin,direction,repeat)
    ...
    def sampleScatterIsotropic( self, ekin, repeat = None ):
        ...
        """
        return _rawfct['ncrystalrel_samplesct_iso'](self._rawobj_scat,ekin,repeat)
    ...
    def generateScattering( self, ekin, direction, repeat = None ):
        ...
        """
        return _rawfct['ncrystalrel_genscatter'](self._rawobj_scat,ekin,direction,repeat)
    ...
    def generateScatteringNonOriented( self, ekin, repeat = None ):
        ...
        """
        return _rawfct['ncrystalrel_genscatter_nonoriented'](self._rawobj_scat,ekin,repeat)
    ...
    def rngSupportsStateManipulation(self):
        ...
        return bool(_rawfct['ncrystalrel_rngsupportsstatemanip_ofscatter'](self._rawobj_scat))
    ...
    def setRNGState(self,state):
        """Set current RNG state.
           ...
           in an error.
        """
        _rawfct['ncrystalrel_setrngstate_ofscatter']( self._rawobj_scat,
                                                   _str2cstr(state) )
...
def createScatterIndependentRNG(cfgstr,seed = 0):
    ...
    rawobj = _rawfct['ncrystalrel_create_scatter_builtinrng'](_str2cstr(cfgstr),seed)
...
def registerInMemoryFileData(virtual_filename,data):
    ...
    _rawfct['ncrystalrel_register_in_mem_file_data'](virtual_filename,data)


#numpy compatible wl2ekin and ekin2wl
_c_wl2ekin = float(_rawfct['ncrystalrel_wl2ekin'](1.0))
_c_ekin2wl = float(_rawfct['ncrystalrel_ekin2wl'](1.0))
...
def wl2ekin(wl):
    ...
    if _np and hasattr(wl,'__len__'):
        ...
        return _rawfct['ncrystalrel_wl2ekin'](wl)
...
def ekin2wl(ekin):
    ...
    if _np and hasattr(ekin,'__len__'):
        ...
        return _rawfct['ncrystalrel_ekin2wl'](ekin)

def clearCaches():
    """Clear various caches"""
    _rawfct['ncrystalrel_clear_caches']()
def clearInfoCaches():
    """Deprecated. Does the same as clearCaches()"""
    clearCaches()
def disableCaching():
    """Disable caching of Info objects in factory infrastructure"""
    _rawfct['ncrystalrel_disable_caching']()
def enableCaching():
    """Enable caching of Info objects in factory infrastructure"""
    _rawfct['ncrystalrel_enable_caching']()

def hasFactory(name):
    """Check if a factory of a given name exists"""
    return bool(_rawfct['ncrystalrel_has_factory'](_str2cstr(name)))
...
def setDefaultRandomGenerator(rg, keepalive=True):
    ...
    """
    _rawfct['ncrystalrel_setrandgen'](rg)
...
def atomDB(Z,A=None,throwOnErrors=True):
    ...
    if obj:
        return obj
    if strkey:
        rawatomdata=_rawfct['ncrystalrel_create_atomdata_fromdbstr'](_str2cstr(key))
    else:
        rawatomdata=_rawfct['ncrystalrel_create_atomdata_fromdb'](*key)
    if not _rawfct['ncrystalrel_valid'](rawatomdata):
...
def browseFiles(dump=False,factory=None):
    ...
    for n,s,f,p in _rawfct['ncrystalrel_get_filelist']():
...
def addCustomSearchDirectory(dirpath):
    ...
    _rawfct['ncrystalrel_add_custom_search_dir'](_str2cstr(str(pathlib.Path(dirpath).resolve())))

def removeCustomSearchDirectories():
    """Remove all search directories added with addCustomSearchDirectory."""
    _rawfct['ncrystalrel_remove_custom_search_dirs']()
...
def enableStandardSearchPath( enable = True ):
    ...
    """
    _rawfct['ncrystalrel_enable_stdsearchpath'](1 if enable else 0)
...
def enableStandardDataLibrary( enable = True, dirpath_override = None ):
    ...
    _rawfct['ncrystalrel_enable_stddatalib'](1 if enable else 0, d)
...
def browsePlugins(dump=False):
    ...
    """
    l=_rawfct['ncrystalrel_get_pluginlist']()
    if not dump:
        return l
...
def debyeIsotropicMSD( *, debye_temperature, temperature, mass ):
    ...
    """
    return float(_rawfct['ncrystalrel_debyetemp2msd'](debye_temperature, temperature, mass))
...
def debyeTempFromIsotropicMSD( *, msd, temperature, mass ):
    ...
    """
    return float(_rawfct['ncrystalrel_msd2debyetemp'](msd, temperature, mass))
