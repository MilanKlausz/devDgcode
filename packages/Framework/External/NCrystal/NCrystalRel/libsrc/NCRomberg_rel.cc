...
void NCrystalRel::Romberg::evalFuncMany(double* fvals, unsigned n, double offset, double delta) const
{
  double * it = &fvals[0];
...
double NCrystalRel::Romberg::evalFuncManySum(unsigned n, double offset, double delta) const
{
  double sum = 0.0;
  double nn = n;//only cast once
  for ( double i = 0; i < nn; ++i )
...
bool NCrystalRel::Romberg::accept(unsigned, double prev_estimate, double estimate,double,double) const
{
  return ncabs(estimate-prev_estimate)<1e-8;
}

void NCrystalRel::Romberg::convergenceError(double a, double b) const
{
  std::cout<<"NCrystal CalcError: Romberg integration did not converge. Will attempt to write"
    " function curve to ncrystalrel_romberg.txt for potential debugging purposes."<<std::endl;
  writeFctToFile("ncrystalrel_romberg.txt", a, b,16384);//2^(maxlevel-2)+1, i.e. last amount of pts sampled at once (not exactly at those precise points though).
  NCRYSTAL_THROW(CalcError,"Romberg integration did not converge. Wrote function curve to ncrystalrel_romberg.txt for potential debugging purposes.");
}

double NCrystalRel::Romberg::integrate(double a, double b) const
{
...
#include <fstream>
#include <iomanip>
void NCrystalRel::Romberg::writeFctToFile(const std::string& filename, double a, double b, unsigned n) const
{
