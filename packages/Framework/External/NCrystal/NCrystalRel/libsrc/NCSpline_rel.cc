...
void NCrystalRel::CubicSpline::set( const VectD& y,
...
void NCrystalRel::SplinedLookupTable::set( const VectD& fvals,
                                        double a,double b,
...
void NCrystalRel::SplinedLookupTable::set( const Fct1D* thefct,
                                        double a,double b,
                                        double fprime_a, double fprime_b,
                                        unsigned npts,
                                        const std::string& name,
...
void NCrystalRel::SplinedLookupTable::producefile( const Fct1D* thefct,
                                                double fprime_a, double fprime_b,
                                                ...
                                                const std::string& userdesc ) const
{
  ...
  if (file_exists(filename)) {
    std::cout <<"NCrystal: Aborted writing of spline data file ncrystalrel_splinedlt_"<<username<<"_??.txt - too many files in current dir."<<std::endl;
    return;
  }

  std::ofstream ofs (filename.c_str(), std::ofstream::out);
  ofs << std::setprecision(20);
  ofs << "#ncrystalrel_splined_lookuptable\n";
  ofs << "#name = "<<name<<"\n";
