#ifndef NCrystalRel_IofQHelper_hh
#define NCrystalRel_IofQHelper_hh
...
#include "NCrystalRel/internal_NCPointwiseDist.hh"
...
inline NCrystalRel::IofQHelper::IofQHelper( const std::pair<VectD,VectD>& QI )
  : IofQHelper(QI.first,QI.second)
{
}

inline double NCrystalRel::IofQHelper::calcQIofQIntegral( NeutronEnergy ekin ) const
{
  if ( ekin >= m_ekinMax )
    return m_normFact;
...
inline double NCrystalRel::IofQHelper::sampleQValue( RNG& rng, NeutronEnergy ekin ) const
{
  constexpr double kkk = 4.0 * ekin2ksq(1.0);
