...
namespace NCrystalRel {
  ...
  struct EnableSharedFromThis : std::enable_shared_from_this<Derived> {
    ...
    //shared pointer, e.g. created with NCrystalRel::makeSO or std::make_shared):
...
#  if __cplusplus >= 201703L
      ...
      if (!shptr)
        NCRYSTAL_THROW( LogicError, "shared_obj_from_this() does not work since this"
                        " instance is not managed by std::shared_ptr / NCrystalRel::shared_obj" );
      return shptr;
#else
