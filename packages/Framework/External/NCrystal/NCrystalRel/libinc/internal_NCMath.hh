#ifndef NCrystalRel_Math_hh
#define NCrystalRel_Math_hh
...
#include "NCrystalRel/internal_NCRomberg.hh"
#include <functional>
...
inline double NCrystalRel::ncmin(double a, double b) { return std::min(b,a); }
inline double NCrystalRel::ncmax(double a, double b) { return std::max(b,a); }
inline double NCrystalRel::ncmax(double a, double b, double c) { return std::max(c,std::max(b,a)); }
inline double NCrystalRel::ncmin(double a, double b, double c) { return std::min(c,std::min(b,a)); }
inline double NCrystalRel::ncmin(double a, double b, double c, double d) { return std::min(std::min(d,c),std::min(b,a)); }
inline double NCrystalRel::ncmax(double a, double b, double c, double d) { return std::max(std::max(d,c),std::max(b,a)); }

inline constexpr unsigned NCrystalRel::ncconstexpr_log10ceil( unsigned val )
{
...
template<class TInt>
inline constexpr TInt NCrystalRel::ncconstexpr_ispow2( TInt a )
{
  return detail::ncconstexpr_ispow2_helper<TInt>(a, 1);
}

template<class TInt>
constexpr TInt NCrystalRel::ncconstexpr_roundupnextpow2( TInt a )
{
  return ncconstexpr_ispow2(a) ? a : ncconstexpr_roundupnextpow2( a + 1 );
}

inline double NCrystalRel::ncabs(double a) { return std::abs(a); }
inline bool NCrystalRel::ncisnan(double a) { return std::isnan(a); }
inline bool NCrystalRel::ncisinf(double a) { return std::isinf(a); }

inline double NCrystalRel::ncclamp(double val, double low, double up)
{
  nc_assert(up>=low);
...
inline NCrystalRel::PairDD NCrystalRel::sincos_2pix( double x )
{
  PairDD res;
...
inline NCrystalRel::PairDD NCrystalRel::sincos_fast( double x )
{
  return sincos_2pix( x * kInv2Pi );
}

inline void NCrystalRel::sincos(double A,double&cosA, double& sinA)
{
  cosA = std::cos(A);
  sinA = std::sin(A);
}

inline void NCrystalRel::sincos_mpi8pi8_lowres(double A, double& cosA, double& sinA) {
...
inline void NCrystalRel::sincos_mpipi(double A, double& cosA, double& sinA) {
...
inline void NCrystalRel::sincos_0pi32(double A, double& cosA, double& sinA) {
...
inline void NCrystalRel::sincos_0pi(double A, double& cosA, double& sinA) {
  //Like sincos_mpipi, but without need to deal with negative A's. Works until 3pi/2.
...
inline void NCrystalRel::sincos_02pi(double A, double& cosA, double& sinA) {
...
inline double NCrystalRel::cos_02pi(double A) {
  nc_assert(A>=0.0&&A<=k2Pi);
  A-=kPi;
  return std::copysign(cos_mpipi(A),ncabs(A)-kPiHalf);
}

inline double NCrystalRel::sin_02pi(double A) {
  nc_assert(A>=0.0&&A<=k2Pi);
  A -= kPi;
  double Aabs = ncabs(A);
...
inline bool NCrystalRel::intervalsOverlap(double a0, double b0, double a1, double b1)
{
  nc_assert(a0<=b0&&a1<=b1);
  return a1<=b0 && a0 <= b1;
}

inline bool NCrystalRel::intervalsDisjoint(double a0, double b0, double a1, double b1)
{
  nc_assert(a0<=b0&&a1<=b1);
  return a1 > b0 || a0 > b1;
}

inline bool NCrystalRel::valueInInterval(double a, double b, double x)
{
  nc_assert(b>=a);
...
inline bool NCrystalRel::valueInInterval( const NCrystalRel::PairDD& ab, double x)
{
  return valueInInterval(ab.first,ab.second,x);
}

inline double NCrystalRel::exp_smallarg_approx( double x )
{
...
inline double NCrystalRel::expm1_smallarg_approx( double x )
{
  //7th order taylor expansion
...
inline double NCrystalRel::exp_negarg_approx( double x )
{
  nc_assert(x<=0.0);
  if (x<-0.1) {
...
inline double NCrystalRel::exp_approx(double x)
{
  return x>0.0 ? 1.0/exp_negarg_approx(-x) : exp_negarg_approx(x);
}

inline double NCrystalRel::atan_smallarg_approx(double x) {
...
inline double NCrystalRel::atan_approx(double x)
{
...
inline NCrystalRel::CosSinGridGen::CosSinGridGen( unsigned n, double offset, double delta, bool small_angles )
  : m_left(n-1),
...
inline bool NCrystalRel::CosSinGridGen::step() {
  if (!m_left)
    return false;
  --m_left;
...
inline void NCrystalRel::StableSum::add( double x )
{
  double t = m_sum + x;
...
inline double NCrystalRel::StableSum::sum() const
{
  return m_sum + m_correction;
}

inline bool NCrystalRel::floateq(double a, double b, double rtol, double atol)
{
...
template <class Func>
inline double NCrystalRel::integrateSimpsons(const Func& f, double a, double b, unsigned n) {
  nc_assert(b>a&&n>1);
  if (n%2==0)
    n+=1;//ensure odd
  StableSum res;
...
template <class Func>
inline double NCrystalRel::integrateTrapezoidal(const Func& f, double a, double b, unsigned n) {
  nc_assert(b>a&&n>1);
  StableSum res;
  const double dx = (b-a)/(n-1);
  res.add(f(a)*0.5);
...
template <class Func>
inline double NCrystalRel::integrateRomberg17(const Func& f, double a, double b) {
...
template <class Func>
inline double NCrystalRel::integrateRomberg33(const Func& f, double a, double b) {
...
template <class Func>
inline double NCrystalRel::findRoot2(const Func& f,double a, double b, double acc)
{
...
inline void NCrystalRel::vectorAppend( NCrystalRel::VectD& v1, const NCrystalRel::VectD& v2 )
{
...
template<class TVector, class Func>
inline TVector NCrystalRel::vectorTrf(const TVector& input, const Func& f)
{
  TVector out;
...
template <class TVector>
inline typename TVector::value_type& NCrystalRel::vectAt(TVector& v, typename TVector::size_type idx)
{
#ifndef NDEBUG
  return v.at(idx);
#else
...
template <class TVector>
inline const typename TVector::value_type& NCrystalRel::vectAt(const TVector& v, typename TVector::size_type idx)
{
#ifndef NDEBUG
  return v.at(idx);
#else
