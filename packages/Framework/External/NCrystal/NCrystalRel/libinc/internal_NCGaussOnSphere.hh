#ifndef NCrystalRel_GaussOnSphere_hh
#define NCrystalRel_GaussOnSphere_hh
...
inline double NCrystalRel::GaussOnSphere::evalXInRange(double x) const
{
  nc_assert(isValid());
...
inline double NCrystalRel::GaussOnSphere::evalXExact(double x) const
{
  nc_assert(isValid());
  return ncabs(x) <= m_truncangle ? m_norm * std::exp( m_expfact * x * x ) : 0.0;
}

inline double NCrystalRel::GaussOnSphere::evalX(double x) const {
  nc_assert(isValid());
  return ncabs(x) <= m_truncangle ? evalXInRange(x) : 0.0;
}

inline double NCrystalRel::GaussOnSphere::evalCosX(double cosx) const {
  nc_assert(isValid());
...
inline double NCrystalRel::GaussOnSphere::evalCosXInRange(double cosx) const {
  nc_assert(isValid());
...
inline double NCrystalRel::GaussOnSphere::circleIntegral( double cg, double sg, double ca, double sa ) const
{
  nc_assert(isValid());
