#ifndef NCrystalRel_Matrix_hh
#define NCrystalRel_Matrix_hh
...
namespace NCrystalRel {
  ...
  Matrix operator*(const Matrix& m1, const NCrystalRel::Matrix& m2);
...
inline NCrystalRel::Matrix::Matrix()
  : m_rowcount(0), m_colcount(0)
{
}

inline NCrystalRel::Matrix::Matrix(MatrixAllowCopy_t, const NCrystalRel::Matrix & o)
  : m_data(SVAllowCopy,o.m_data), m_rowcount(o.m_rowcount), m_colcount(o.m_colcount)
{
}

inline NCrystalRel::Matrix::Matrix(NCrystalRel::Matrix && o)
...
inline NCrystalRel::Matrix& NCrystalRel::Matrix::operator=(NCrystalRel::Matrix&& o)
{
  m_data = std::move(o.m_data);
  m_rowcount = o.m_rowcount;
  m_colcount = o.m_colcount;
...
inline NCrystalRel::Matrix::Matrix(unsigned row, unsigned col, Span<const double> data)
{
...
inline const double* NCrystalRel::Matrix::operator[](unsigned irow) const
{
  nc_assert( irow < m_rowcount) ;
  return &(m_data[0]) + irow * m_colcount;
}

inline NCrystalRel::Matrix NCrystalRel::Matrix::operator* (double factor)
{
  Matrix res;
...
inline bool NCrystalRel::Matrix::isEqual(const NCrystalRel::Matrix& o) const
{
...
inline NCrystalRel::Matrix NCrystalRel::Matrix::operator~() const
{
  Matrix res;
  res.m_colcount = m_colcount;
  res.m_rowcount = m_rowcount;
...
inline void NCrystalRel::Matrix::transpose()
{
...
inline unsigned NCrystalRel::Matrix::nRows() const
{
  return m_rowcount;
}

inline unsigned NCrystalRel::Matrix::nCols() const
{
  return m_colcount;
}

inline double NCrystalRel::Matrix::mag2() const
{
  double sum(0.0);
  for ( auto e : m_data )
    sum += e*e;
...
inline NCrystalRel::Matrix& NCrystalRel::Matrix::operator*=(const double& f)
{
  for (auto& e : m_data)
    e *= f;
  return *this;
...
inline void NCrystalRel::Matrix::unit()
{
  double themag2 = mag2();
  if (themag2==1.0)
    return;
